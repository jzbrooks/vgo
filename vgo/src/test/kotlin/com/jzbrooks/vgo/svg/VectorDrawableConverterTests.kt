...
class VectorDrawableConverterTests {
    ...
    fun testRootElementAttributesAreConverted() {
        val svg = ScalableVectorGraphic(emptyList(), topLevelAttributes)

        val vectorDrawable = svg.toVectorDrawable()

        assertThat(vectorDrawable.attributes.id).isEqualTo("visibilitystrike")
    ...
    fun testPathElementAttributesAreConverted() {
        ...
        assertThat(first.attributes.id).isEqualTo("heart")
    ...
    fun testContainerElementAttributesAreConverted() {
        ...
        val svg = ScalableVectorGraphic(
            listOf(Group(emptyList(), Group.Attributes("transform_group", transform, mutableMapOf()))),
            topLevelAttributes
        )
        ...
        assertThat(first.attributes.id).isEqualTo("transform_group")
        assertThat(first.attributes.transform).isEqualTo(transform)
    }
    ...
    fun testClipPathsAreConverted() {
        val svg = ScalableVectorGraphic(
            listOf(
                ClipPath(
                    ...
                    ClipPath.Attributes("_clip1", mutableMapOf())
                ),
                Group(
                    listOf(
                    ...
                    Group.Attributes(null, Matrix3.IDENTITY, mutableMapOf("clip-path" to "url(#_clip1)"))
                )
            ),
            topLevelAttributes
        )
        ...
        assertThat(innerGroup.attributes.isEmpty()).isTrue()
    }
    ...
    fun testRgbColorIsConverted() {
        ...
        assertThat(path.attributes.foreign).contains("android:strokeColor" to "#170017")
    }
}
