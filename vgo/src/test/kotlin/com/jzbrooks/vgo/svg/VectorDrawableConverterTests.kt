...
class VectorDrawableConverterTests {
    private val graphic = ScalableVectorGraphic(
        emptyList(),
        "visibilitystrike",
        mutableMapOf(
    ...
    fun testRootElementAttributesAreConverted() {
        val vectorDrawable = graphic.toVectorDrawable()

        assertThat(vectorDrawable.id).isEqualTo("visibilitystrike")
        assertThat(vectorDrawable.foreign).containsOnly(
    ...
    fun testRootElementStyleAttributeIsConverted() {
        val svg = graphic.copy(
            foreign = graphic.foreign.toMutableMap().apply {
    ...
    fun testPathElementAttributesAreConverted() {
        ...
        assertThat(first.id).isEqualTo("heart")
        assertThat(first.foreign).isEqualTo(
            mapOf(
        ...
        val svg = graphic.copy(
            elements = listOf(
                Path(
                    ...
                    "heart",
                    mutableMapOf("style" to "stroke:red"),
                    Colors.TRANSPARENT,
                )
            )
        )
    ...
    fun testContainerElementAttributesAreConverted() {
        ...
        val svg = graphic.copy(elements = listOf(Group(emptyList(), "transform_group", mutableMapOf(), transform)))

        val vectorDrawable = svg.toVectorDrawable()

        val first = vectorDrawable.elements.first() as Group
        assertThat(first.id).isEqualTo("transform_group")
        assertThat(first.transform).isEqualTo(transform)
    }
    ...
    fun testClipPathsAreConverted() {
        val svg = graphic.copy(
            elements = listOf<Element>(
                ClipPath(
                    listOf(
                        Path(
                            CommandString("""M859.613,320.594L294.248,265.525L284.349,367.149L849.714,422.218L859.613,320.594Z""").toCommandList(),
                            null,
                            mutableMapOf(),
                            Colors.BLACK,
                        )
        ...
        assertThat(innerGroup.id).isNull()
        assertThat(innerGroup.transform).isSameAs(Matrix3.IDENTITY)
        assertThat(innerGroup.foreign).isEmpty()
    }
    ...
    fun testRgbColorIsConverted() {
        ...
        assertThat(path.foreign).contains("android:strokeColor" to "#170017")
    }
}
